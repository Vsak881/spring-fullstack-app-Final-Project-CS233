    ...
    try {
        ...
        if (res.ok) {
            ...
            document.getElementById('auth-links').innerHTML =
                `<span style="color:var(--accent);font-size:0.85rem">${currentUser.username}</span>
...
function addToCart(productId, name, price) {
    ...
    if (existing) {
        ...
        cartItems.push({ productId, name, price, quantity: 1 });
    }
    ...
    container.innerHTML = cartItems.map((item, idx) => `
        <div class="cart-item">
            ...
            <div style="display:flex;align-items:center;gap:1rem">
                <span style="font-family:var(--font-display);font-size:1.2rem;color:var(--text)">
    ...
    try {
        ...
        document.getElementById('admin-products-list').innerHTML = products.map(p => `
            <div class="admin-product-row">
                <div>
                    <div style="color:var(--white)">${p.name}</div>
        ...
        document.getElementById('admin-orders-list').innerHTML = orders.map(o => `
            <div class="admin-product-row">
                <div>
                    <div style="color:var(--white)">Order #${o.id}</div>
                    <div style="color:var(--text-muted);font-size:0.8rem">$${o.totalPrice} — ${o.status}</div>
                </div>
                <span class="order-status">${o.status}</span>
            </div>
        `).join('') || '<div style="color:var(--text-muted);padding:1rem">No orders yet</div>';
    } catch (e) {
...
async function addProduct() {
    ...
    try {
        ...
        if (res.ok) {
            document.getElementById('admin-msg').textContent = 'Product added successfully!';
            loadAdmin();
